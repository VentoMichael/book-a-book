...
@section('content')
    ...
    <form method="POST" action="/users/{{$admin->first()->name}}" enctype="multipart/form-data">
        ...
        <div class="field">
            ...
            <img src="{{ asset('storage/'.$admin->first()->file_name) }}" alt="Photo de profil de {{$admin->first()->name}}">
        ...
        <div class="field">
            ...
            <input type="email" name="email" class="@error('email')is danger @enderror input" id="email">
            <p>{{$errors->first('email')}}</p>
        </div>
        <div class="field">
            <label for="password" class="label">Modifier mon mot de passe</label>
            <input type="password" name="password" class="@error('password')is danger @enderror input" id="password">
